...
const BASE_PATH = '/sequences/ladies/desktop/';
...
export const LadiesSequence: React.FC = () => {
  ...
  // Canvas fades in (0 to 0.05) over the previous section
  const canvasOpacity = getOpacity(0.0, 0.05);

  // UI elements fade in (0.05 to 0.25)
  const getUIOpacity = (inStart: number, inEnd: number) => {
    return Math.min(Math.max((progress - inStart) / (inEnd - inStart), 0), 1);
  };

  const labelOpacity = getUIOpacity(0.05, 0.1);
  const labelY = getY(0.05, 0.1, 20);

  const headlineOpacity = getUIOpacity(0.1, 0.15);
  const headlineY = getY(0.1, 0.15, 40);

  const paragraphOpacity = getUIOpacity(0.15, 0.2);
  const paragraphY = getY(0.15, 0.2, 40);

  const buttonOpacity = getUIOpacity(0.2, 0.25);
  const buttonY = getY(0.2, 0.25, 40);
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      scrub: 0.5,
      onUpdate: (self) => {
        // Frames finish by 0.9 so the final frame holds while it crossfades to Unisex
  ...
  return (
    <>
      {/* Added id="ladies" for snap targeting */}
      <div id="ladies" ref={containerRef} className="relative w-full h-[400vh] bg-transparent -mt-[100vh] z-20">
        <div ref={stickyRef} className="sticky top-0 left-0 w-full h-screen overflow-hidden bg-black">

          {/* Canvas with dynamic opacity for fade-in from darkness */}
          <canvas
            ref={canvasRef}
